...
abstract: |
  This paper examines the effectiveness of facial detection APIs on broadcast video stills of Australian Open tennis matches. The aim was to evaluate the performance of several APIs to determine the best API to use for face detection of players throughout a match. For training purposes, faces were manually tagged in 6406 images, using a specially constructed web application, recording the scene characteristics and features of individual faces, such as head wear or sunglasses. API performance was evaluated on their success rate at detecting the tagged faces, and also relative face and scene attributes. Google found 1319 faces, this amount is 0.57 of the manually tagged faces, it also found an additional 638 relevant faces that were not tagged manually. A logistic regression was used to determine the effects of the scene characteristics and features of individual faces on the likelihood of an API detecting the face. It showed all APIs were significantly affected by features of individual faces, and not all were significantly impacted by scene attributes. It would be expected that, in future, utilising cameras with a focus on the player's faces would be necessary to improve detection of faces and minimise impacts of features of individual faces.
...
Images from the Australian Open 2016 were provided by Tennis Australia. There were 6406 images, 800 $\times$ 450px.
The images were derived from 5 minute segments taken from 105 video files, with a still shot stored every three seconds. The video files used were the broadcasts of the tennis matches shown on the Seven Network during the Australian Open 2016. The sample included an equal amount of females and males singles tennis matches.
Images contained the faces of many people, such as players, staff on the court and fans in the crowd. 
...
kable(SceneAttributes, format = "latex", longtable=FALSE, booktabs =TRUE, caption = "Summary of extracted scene information for match images.")
```
...
kable(FaceAttributes, format = "latex", longtable=FALSE, booktabs = TRUE, caption = "Summary of extracted face information for match images")
```
...
The face recognition software APIs were accessed via a R [@R] script, calling the APIs relied on the httr [@httr] package. Special handling was required due to the limits on requests per minute of Skybiometry and Microsoft, time-controlling and error management of the requests was incorporated for these APIs.

Figure \ref{fig:timeFigure} shows the distribution of processing time required by the APIs for each image. The violin plot display created by @Violin 'combines the box plot and the density'. A categorical variable splits the data along the x axis, as would be seen in a box plot display. A density plot is then incorporated on the y axis. This is produced by manipulating a density display; switching the axes allows the continuous variable to increase along on the y axis, then the violin shape is created by mirroring the density. The width of the violin will now be twice the length that would be have been measured vertically in a density plot. The lines overlaid in each plot represent the smoothed linear model of the amount of faces, it can be seen that all APIs take longer to complete detection of faces in an image when there are more faces.


As seen in the facets of Figure \ref{fig:timeFigure} times taken to search are varied between APIs, but Skybiometry consistently spent less time than other APIs. Microsoft and Google's times vary regardless of the number of faces found. 
...
Descriptive statistics were used to discern differences in the performance of the four APIs. These are presented in tables and plots and discussed in the results section. A logistic regression was used to study the effect of conditions and accessories on the detection performance of the APIs. The estimates shown indicate the magnitude of the impact on the detection rate, and the symbols shown indicate the significance, or reliability, of the estimated impact.
Proportions were used a simplistic comparison to indicate the amount of the total faces that were detected by the software. Faces were grouped by the softwares that detected them, these groups and the attributes of the faces within them show the common features that contributed to a face being detected.
Mosaic plots also examine faces that posessed a combination of features. Two way feature intersections show the common intersections of manually tagged faces.
Some examples of intersections across four variables are displayed in a table format, only the most common feature combinations were displayed.
...
The manual tagging helped to provide a benchmark for faces found by APIs. The face detection rate indicated what attributes of an image or face were associated with face detection by an API. 
...
#modSummaries <- map(TypeGlms, broom::glance)
...
kable(ModelEstimatesTable, format = "latex", align=c('l', 
                                           ...
                                           'r', 'l',
                                           'r', 'l'),
             booktabs = TRUE, longtable=FALSE, caption="Evaluation of factors affecting face recognition for API, as assessed by logistic regression model. Coefficients from the four models shown (significance: ** 0.01 * 0.05 . 0.01 - NS). Most APIs were affected by similar factors, with glasses, visor or hat, head angle, lighting, shot angle and situation all contributing significantly to face detection.")
```


For the most part, the APIs were all affected by the same factors.
The factors that negatively affected all APIs were glasses, visor or hat. Background only negatively affected Google's detection rate. Situation had different effects on each API.
When the shot angle was considered in conjunction with the background the results for Google showed there was a positive effect on detection rate of faces in all circumstances. There was no significant impact on the other APIs.


```{r UnlikelyFace, fig.cap = "The face captured manually in this image was not captured by any API. The head angle, shot angle, lighting, wearing the visor and being captured across the court during the match are all associated with lower rates of detection.\\label{fig:UnlikelyFace}", dev="jpeg", dpi=300}
...
When forcing no intercept, all variables are significant at the 0 level, and obscured was significant for all four up to the 0.01 level. 
...
This paper has compiled results from applying commonly available software for face detection to a set of stills from tennis match video. The set of detected faces from each software were able to be contrasted against manually tagged faces and those found by other software. This combined set has allowed the performance of the software to be analysed under a range of circumstances. The varying accessories, camera angles and scene attributes noted for each manually tagged face allow a reference point to determine how the detection rates of the software are influenced. If an image set shows similar features to those exhibited in the Australian Open broadcast stills this will highlight the impacts the features will have on the software's ability to produce relevant detections.

If an image set utilised a camera angle that captured a person's profile, using Animetrics, Microsoft or Skybiometry would significantly reduce the rate of detection. The logistic regression contributed to understanding the impacts of face and scene features in a measurable way. For each software it utilised the set of manually tagged faces to analyse impacts on detection. This analysis showed 

A long term goal is to detect emotional state of the player, and track this through a match, to study the effect on performance. Several of the face recognition APIs also report an emotional state, but our analysis of this data suggests it is unreliable. 
