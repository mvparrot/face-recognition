...
J <- full_join(S0, S1, by = "FileName")
J1 <- full_join(J, S2, by = "FileName")
J2 <- full_join(J1, S3, by = "FileName")

J2 <- select(J2, FileName, Skybiometry = SEmotion,
             Microsoft = MEmotion, Google = GEmotion)
 
write.csv(J2, "DominantEmotions.csv", row.names = FALSE)
...
```{r GoogleJoy}
Gjoy <- GEP1 %>% filter(Emotion=="joy")

Gjoylist<- as.list(Gjoy$FileName)

Mjoy <- MEP1 %>% filter(FileName %in% Gjoylist)
