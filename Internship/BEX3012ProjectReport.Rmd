...
GEP1 <- GEP %>% group_by(FileName) %>% 
  gather(key = Emotion, value=EmotionValue, 
         anger, joy, sorrow, surprise,neutral)  %>% 
  arrange(FileName, desc(EmotionValue)) %>%
      group_by_(~ FileName) %>% 
      slice(1)

SBP1 <- SBP %>% group_by(aname) %>% 
  gather(key = Emotion, value=EmotionValue, 
...
```{r Regression, fig.height=9.5, fig.width=7.5, fig.cap = "The logistic regression returned statistically significant results when the face was captured in the presence of a graphic. \\label{Regression}"}
...
SignificancePlot <- function(model, data = ModelTable(model)) {
  data %>%
  mutate(significant = `Pr(>|t|)` < 0.05) %>%
  mutate(`Pr(<|t|)` = 1 - `Pr(>|t|)`) %>%
  ggplot(aes(x=variable, y=`Pr(<|t|)`)) +  
  theme(axis.text.x = element_text(angle = 30, hjust = 1)) +
  geom_col(aes(fill=significant))
}

EstimatesPlot <- function(model, data = ModelTable(model)) {
  data %>%
  mutate(significant = `Pr(>|t|)` < 0.05) %>%
...
mod <- glm(neutral01 ~ headangle + shotangle, data = GEP1)


ModelPlotResults(mod)
