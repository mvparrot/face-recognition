...
overlayGgplot <- function(imgList, mergeData, matchBox=FALSE){
  for(i in imgList){
    ...
    if(matchBox){
      p <- p + geom_path(aes(x=x, y=450-y, group = type, colour=boxID), faceData) + guides(colour="none")
    }
    else{
...
####Data Processing
The data was spread across 6 files. For each software we had the classified faces csv file, these are located in the appendix.
...
```{r Face-Bounding-Box-Data, results='hide', type="figure", fig.cap='..', anchor="figure"}
...
```{r G4 Face Bounding Box Data, anchor="figure"}
