...
knitr::opts_chunk$set(echo=FALSE, message=FALSE, warning=FALSE, kfigr.link=TRUE, kfigr.prefix=TRUE, cache=TRUE, fig.env = TRUE, fig.)
...
ggplot(times) + 
  ...
  scale_colour_manual("API", values=type.colours)
```
...
To do so, we gathered the name of the file the face was found in and the API that found the potential face. The automatically determined time values were also included. The minimum and maximum x and y values were drawn from different values in each software's CSV files. This required some processing to align the differing values to be comparable.
...
It is important that the selected API be able to return strong and consistent results for player's faces.
