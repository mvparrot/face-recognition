...
```{r ModelEstimates, fig.cap ="\\label{tab:Image-Characteristics-Table}"}
GlmModelEstimates(model = hit ~ 0 + 
              shotangle + 
              bg + 
              bg*shotangle + 
              situation + 
...
ModelEstimatesTable <- as.data.frame(cbind(ModelEstimatesTypes, ModelEstimatesSign[,2:5])) %>% 
  unite(Animetrics, A.e, A.s, sep="") %>% 
  unite(Google, G.e, G.s, sep="") %>% 
  unite(Microsoft, M.e, M.s, sep="") %>% 
  unite(Skybiometry, S.e, S.s, sep="")

  
knitr::kable(ModelEstimatesTable, align=c("l", "r", "r", "r", "r"),
             booktabs = TRUE, longtable=TRUE, caption="", format = "latex")
```


The variables were kept if they were significant.
...
Table 4, the comparison of Skybiometry and Manually annotated faces gives the lowest amount in this regard. Table 5, Google, found the largest amount of faces that matched those manually annotated.
...
Examples of tricks of colours are shown in Figure c, where logos on the shirts were eturned as faces.
...
We then considered that there would be an uneven amount of faces with certain image attributes. This is considered in the mosaics in Figure \@ref(fig:MosaicShotangle) and Figure \@ref(fig:MosaicSit).
...
```{r glasses, fig.cap = "This cross table compares the amounts and percentages of faces captured and whether the person was wearing glasses. The first row is the faces that were tagged as not wearing glasses, the second row is faces that were. For the API to not have been affected by the prescence of glasses, the proportion should be similar to that found manually. Animetrics had the highest proportio difference, 93.31% of the faces it found did not have glasses. Google is the closest proportion, 88.62% of the faces it found were wearing glasses.\\label{tab:glasses}"}
...
* bookdown [@bookdown] and knitr [@knitr] created the pdf output
* ggplot2 [@ggplot2] was used to make the plots 
* pander [@pander] printed tables
* knitcitations [@knitcitations] and RefManager [@RefManager] allowed for references to be integrated
