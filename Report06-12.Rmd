...
times <- amin %>%  
  ...
  group_by(file, type) %>% 
  mutate(n=dplyr::n(), time.user.self=first(time.user.self), time.sys.self=first(time.sys.self))  
...
The upper left quadrany of tables shows the faces found manually and by each API. The ideal set would have majority of faces found belong to this category. 
...
i1 <- "2016_CT6_R01_CGarcia_FRA_vs_BStrycova_CZE_WS145_clip.0015.png" %>% overlayGgplot(., legend=FALSE) + ggtitle("  Figure a.")
i2 <- "2016_HSA_R03_FDelbonis_ARG_vs_GSimon_FRA_MS302_clip.0072.png" %>% overlayGgplot(., legend=FALSE) + ggtitle("  Figure b.")
i3 <- "2016_CT6_R02_ABeck_GER_vs_TBacsinszky_SUI_WS220_clip.0017.png" %>% overlayGgplot(., legend=FALSE) + ggtitle("  Figure c.")
i4 <- "2016_SC2_R01_ATomljanovic_AUS_vs_KBondarenko_UKR_WS1112_clip.0053.png" %>% overlayGgplot(., legend=FALSE) + ggtitle("  Figure d.")
...
#aminFaces$shotangle 
aminFaces$shotangle <- factor(aminFaces$shotangle, levels = c("Birds Eye","Upward Angle","Player Shoulder Height"))
...
MosaicShotangle<- ggplot(data=aminFaces) + 
  ...
  ggtitle("Plot a. Proportion of faces captured at each angle")
```
...
MosaicSit<- ggplot(data=aminFaces) + 
  ...
  ggtitle("Plot b. Proportion of faces captured in each situation")
```
...
sbbgshot <- ggplot(data=aminFaces) +
  ...
  ggtitle("Plot c. Proportion of faces captured at each angle \nwith various backgrounds")
...
sbbgsit <- ggplot(data=aminFaces) +
  ...
  ggtitle("Plot d. Proportion of faces captured in each situation \nwith various backgrounds")
```
...
```{r ICFigs, fig.width = 8, fig.height = 8, fig.cap = "These plots indicate the proportion or amount of faces with certain attributes, or attribute combinations.\\label{fig:ICFigs}"}

grid.arrange(MosaicShotangle, MosaicSit, sbbgshot, sbbgsit,
             nrow=2, ncol=2)
```
