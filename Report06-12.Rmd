...
knitr::opts_chunk$set(echo=FALSE, message=FALSE, warning=FALSE, kfigr.link=TRUE, kfigr.prefix=TRUE)
...
The sample included images that contained the faces of many people, this included players, staff on the court and fans in the crowd. These faces were included in the manual annontations as they were likely to be found by the softwares selected. To be able to contrast the abilities of the softwares and provide information on how to differentiate between players and other people for further research the sample was not filtered at this stage.
...
We were able to try the online demos to see whether these softwares were viable, we used the following  `r figr("Trial-Image", TRUE, type="Figure")`:


```{r Trial-Image, fig.cap="This is a player", anchor="figure"}
...
```{r Scene-Attributes-Table, anchor="table", fig.cap="Attributes of a Scene of an image. The most appropriate option from the list was selected for each attribute."}
kable(SceneAttributes, format = "pandoc", booktabs = TRUE)
```


The aim was to collect specific information on each different face within the scene. To determine which of the sometimes many faces in the scene it would be reasonable for softwares to detect a standard was created for reasonable detection. This was based on the attributes provided in `r figr("Solution-Specifications-Table", TRUE, type="Table")`, below:

```{r Solution-Specifications-Table, results='asis', anchor="table"}
kable(SolutionSpecs, format = "pandoc", booktabs = TRUE, caption = "Software solutions specifications.")
...
```{r Face-Attribute-Tables, results='asis', anchor="table"}
kable(FaceAttributes, format = "pandoc", booktabs = TRUE, caption = "Attributes of an individual face within an image. The most appropriate option from the list was selected for each attribute.")
```


####Manual Annotation
To record the selections of attribute values for each face and scene a Shiny @shiny App was created. We called this Application our ManualClassificationProgram. This helped to provide information for all attributes quickly and consistently.
The program worked by presenting an image, using the imager @imager package, from the sample of images that had not yet been considered, appearing underneath were a set of radio buttons corresponding to the Scene Attibutes in `r figr("Scene-Attributes-Table", TRUE, type="Table")` as shown above.
...
To find whether the softwares were recognising the same faces a function was created. As the location and size of the boxes around the faces were recorded, these values were used to see if a particular identified face box matched a manually identified face, or a region found by another software.
...
Using the data set^[ALLmetaIMG] of the combined results, we were able to compare the performance of the softwares.
Firstly, we considered how many individual faces the softwares were able to detect in 
`r figr("Face-Bounding-Box-Data", TRUE, type="Figure")` .


## Results


```{r Face-Bounding-Box-Data, results='hide', type="figure", fig.cap="Face Bounding Box Data:", anchor="figure"}
...
The bar chart in 
`r figr("Face-Bounding-Box-Data", TRUE, type="Figure")` above shows the number of Bounding Boxes produced by each software, comparing the height of the bars indicates that Google's Facial Recognition software recognised almost 1000 more faces than the next best software, Microsoft.


```{r Face-Bounding-Box-Data-Box, anchor="figure"}
...
```{r I2, results=TRUE, fig.cap="2016_CT6_R02_JSousa_POR_vs_SGiraldo_COL_MS231_clip.0017.png", anchor="figure"}
...
```{r I3, results=TRUE, fig.cap="2016_SC2_R01_ATomljanovic_AUS_vs_KBondarenko_UKR_WS1112_clip.0053.png", anchor="figure"}
...
```{r I4,results=TRUE, fig.cap="2016_RLA_R04_NDjokovic_SRB_vs_GSimon_FRA_MS401_clip.0003.png", anchor="figure"}
...
```{r I5, results=TRUE, fig.cap="2016_SC2_R01_AKerber_GER_vs_MDoi_JPN_WS1332_clip.0083.png", anchor="figure"}
...
```{r Type-Matches-Manual, anchor="table"}
...
```{r Image-Characteristics-Table, anchor="table"}
ImageCharacteristics <- ALLmetaIMG %>% filter(type=="Manual") %>% group_by(situation, bg, shotangle, detect) %>% dplyr::summarise(count=n())
ImageCharacteristics<-arrange(ImageCharacteristics, desc(count))
ImageCharacteristics<-ImageCharacteristics[1:10,]
kable(ImageCharacteristics, booktabs = TRUE)
```


`r figr("Image-Characteristics-Table", TRUE, type="Figure")`
 displays the feature combinations that produced the most potential face Bounding Boxes recognitions by all four softwares. The most common shot is a crowd shot.
...
```{r Typical-Best-Recognised, results=FALSE, anchor="figure"}
...
```{r Google-Errors-Characteristics-Table, anchor="Table"}
library(dplyr)
...
kable(GoogleNoMatch[1:10,], type = 'table')

#overlayGgplot(GoogleImages, GoogleNoMatch, matchBox = TRUE)
```

`r figr("Google-Errors-Characteristics-Table", TRUE, type="table")`
...
The best scenes for facial recongition have been found, given this information the following `r figr("Face-Characteristics-Table", TRUE, type="table")`
 considers the Characteristics of the individual faces found within those scenes. For this section we chose to consider only the faces that were manually annotated as players, with the intention of not basing results on recognitions of undesirable faces.



```{r Face-Characteristics-Table, anchor="Table"}
...
kable(FaceCharacteristics, type = 'table')
```

`r figr("Face-Characteristics-Table", TRUE, type="table")`
utilises a set of faces that were Manually annotated and also found by Google. 
...
```{r Glasses-Figure, anchor="Figure"}
...
```{r Headangle-Figure, anchor="Figure"}
...
```{r Google-Characteristics-Table, anchor="table"}
...
`r figr("Google-Characteristics-Table", TRUE, type="table")`
shows how many images with potential player's faces Google recognised. This diplays a vast gap between the amount of potential faces found given the different shot angles. Shoulder height is an optimum angle for a Face Bounding Box.
...
It should also be considered that these are softwares focussed on providing recognition in certain controlled scenarios.
If the study was controlled to focus on certain camera angles that align with the facial angles these security programs are intended to recognise faces in.
...
There is the possibility that we could create another app that shows the Facical Bounding Boxes identified by each program, this would allow the annotator to confirm manually whether or not these are faces.


Given that certain Scene attribute combinations produced more facial recognitions than other combinations we should consider limiting the sample of images sent to Google Vision API. This would not only reduce cost but also provide a greater level of detail of the emotions felt by a player during a match. To provide a greater level of information at all points in a match it would be beneficial to derive images from a single camera feed. This feed should match the Scene attributes that provided the most Google faces.
