...
This paper examines the effectiveness of facial detection APIs on broadcast video stills of Australian Open tennis matches. The goal is determine the best API to use for face detection of players throughout a match.
...
```{r Trial-Image, fig.cap = "This image of Bernard Tomic was chosen as a trial image to be presented to each of the software before they were included in the research. It was expected that the software would be able to find this face, despite the player facing away from the camera.\\label{fig:Trial-Image}", message=TRUE, warning=TRUE, dev="jpeg", dpi=300}
...
A 'face' is considered to be any area designated manually or by an API as an area that encloses a human face. These may or may not be actual faces.
...
The bar charts of the situations show that the situation is important in influencing the detection of a face. When the face is accessorised with them, all APIs have a significantly increased chance of finding the face in comparison to the base rate category of the "Match play". While the image being a Graphic had a large impact it was not significant, this is likely because of the small number of images with this attribute. It also shows that the shot angle is only significant for one or two APIs at each level.
...
knitr::kable(ImageCharacteristics, booktabs = TRUE, longtable=TRUE, format.args=list(width=8), caption="This table outlines the combinations of image attibutes that are most common in the image set. The combination of the Logo Wall background and the Player Shoulder Height angle are shared by three of the five image combinations.", format = "latex")
```
...
aminFaces %>% filter(matchesManual) -> afm
...
pander(glassesdf, caption = "This cross table contrasts the proportion of faces captured and whether the person was wearing glasses. The first row is the faces that were manually tagged as not wearing glasses, the second row is faces that were. For an API to not have been affected by the prescence of glasses, the proportions for both categorys should be similar. Animetrics had the highest proportion difference,  of the faces it found did not have glasses. Google is the closest proportion,  of the faces it found were wearing glasses.")
```
...
The APIs, the scripts used to access them, and the resulting data set are contained in the supplementary materials, available at [https://github.com/mvparrot/face-recognition](https://github.com/mvparrot/face-recognition). The APIs evaluated were:
