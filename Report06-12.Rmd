...
knitr::opts_chunk$set(echo=FALSE, message=FALSE, warning=FALSE, kfigr.link=TRUE, kfigr.prefix=TRUE, cache=FALSE, fig.env = TRUE)
...
The amount of time taken for each software to process facial detetion on the individual images is presented in the histogram below:

```{r timeDensity, fig.cap="This density plots diplays the common lengths of time each API took to detect faces in each image. It can be seen that the\\label{fig:timeDensity}"}
times <- ALLmetaIMGnamed %>% filter(!duplicated(file), type!="Manual")
...
To consider how many Type II errors occurred, where a potential face was detected incorrectly, we look at the potential faces that do not match manually annotated faces.
...
## False Discoveries
Below are some examples of when faces were only found by one of the APIs, and how the APIs produced unusual false discoveries.

```{r UnusualImages, fig.cap = "Animetrics provided results that were quite unusual. The top left image presents a potential faces that actually defines an area containing a player's back and a ballboy's torso.  \\label{fig:UnusualImages}" }
