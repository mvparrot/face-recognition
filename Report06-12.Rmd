...
kable(SceneAttributes, format = "markdown", booktabs = TRUE, caption = "Attributes of a Scene of an image. The most appropriate option from the list was selected for each attribute.")
```
...
kable(SolutionSpecs, format = "markdown", booktabs = TRUE, caption="")
```
...
kable(FaceAttributes, format = "markdown", booktabs = TRUE)
```
...
```{r Regression, fig.cap = "\\label{Regression}"}
hitmiss <- function(x){
...
glmFits <- ALLmetaIMGFaces %>%
  ...
  map_df(~ hitmiss(.)) %>%
  split(.$type) %>%
  map(~ glm(hit ~ ., data = select(., -type, -file), family=binomial(link='logit')))
...
kable(ImageCharacteristics, booktabs = TRUE, caption="", format = "markdown")
```
...
kable(GoogleNoMatch[1:10,], caption="", format = "markdown")
...
kable(FaceCharacteristics, caption="", format = "markdown")
```
...
kable(Accuracy, caption="", format = "markdown")
...
kable(GoogleCharacteristics, format = "markdown", booktabs = TRUE, caption="")
```
