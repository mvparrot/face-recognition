...
Regression analysis showed that the situation attribute of an image was most influential in increasing the likelihood of a hit.
And head angle was most influential in decreasing.
...
glmFits <- ALLmetaIMGFaces %>%
  ...
  map_df(~ hitmiss(.)) %>%
  split(.$type) %>%
  map(~ glm(hit ~ . + visorhat*glasses, data = select(., -type, -file), binomial(link = "logit")))
...
```{r, Regression-Significance-Plot, fig.cap = "\\label{Regression--Significance-Plot}"}
...
The bar chart of the interaction of Glasses and Visors in \@ref(Regression--Significance-Plot) or Hats shows that when the face is accessorised with both, Google has a significantly increased chance of finding the face 

```{r, Regression-Estimates-Plot, fig.cap = "\\label{Regression--Estimates-Plot}"}
...
#parallel coordinate plot, vars on x????
#unpack intercept
#don't force to zero
#convert into full model decription - every single level combination
#have every level and cross level - order combinations by p value
#function for estimates for each treatment
#multinomail 
#add hit or miss column, to utilise caret package
```



```{r Regression-Situation-Table, fig.cap = "\\label{Regression-Situation-Table}"}
...
```{r Regression-Angle-Table, fig.cap = "\\label{Regression-Angle-Table}"}
