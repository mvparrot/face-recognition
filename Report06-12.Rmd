...
We were able to try the online demos to see whether these software were viable, we used the following Image displayed below in Figure \@ref(fig:Trial-Image):
...
##Manual Annotation
To record the details of attributes for each face and scene a Shiny [@shiny] App was created. We called this Application our ManualClassificationProgram^[https://github.com/mvparrot/face-recognition/master/ManualClassificationProgram.R]. This helped to provide information for all attributes quickly and consistently.
...
The amount of time taken for each software to process facial detection on the individual images is presented in the histogram below:

```{r timeDensity, fig.cap="This density plot diplays the lengths of time each API took to detect faces in each image. Most of the images take less than one second to process. There is an outlier for Skybiometry that took almost 5 seconds to process. \\label{fig:timeDensity}"}
...
```{r Regression, fig.width=7.5, fig.cap = "The figure depicts the change in the probability of a face being detected by the APIs given a certain attribute level associated with the face. Wearing either glasses, or visor or a hat significantly decreased the likelihood of a manually annotated face being detected by the APIs.\\label{fig:Regression}"}
# Prepare data
library(purrr)
library(tidyverse)
library(gridExtra)
...
## Amount of Potential Faces

Each API returned areas that indicated the potential location of a face. These areas are defined using the four points returned by the APIs.. The Google Vision API detected a large amount of potential faces, much more than Microsoft, Skybiometry or Animetrics. This can be seen below in Figure \@ref(fig:PotentialFaceData).


```{r PotentialFaceData, fig.cap = "\\label{fig:PotentialFaceData}  Potential Face Boxes Per API: The bar chart shows the number of Potential Face Boxes produced by each API, comparing the height of the bars indicates that Google's Facial Recognition API recognized almost 1000 more potential faces than the next best API, Microsoft."}
...
Figure \@ref(fig:PotentialFaceData), the bar chart of the potential faces shows Google produces the most detections. Having a high volume of detected faces returned is valuable for the future use of applying facial recognition during sports.
...
Figure \@ref(fig:PotentialFaceUpset) helps to understand faces that were found by a combination of APIs, and the manual annotations.
The black bubbles below the bars show the API combination that detected all the faces counted in the bar above.


  

```{r PotentialFaceUpSet, fig.cap = "\\label{fig:PotentialFaceUpset} potential facees Per API Combination. The Bar Chart shows the potential facees that were recognised by multiple API or found manually. The largest group, with 809 faces, is potential facees only found by Manual annotations. The following group were the 716 faces recognized both Manually and by the Google API. These combinations may give some indication as to the circumstances when some APIs perform better than others."}
createUpSet(ALLmetaIMGnamedFaces)
```



## Potential Faces Matching Manual Annotations

We have two possible results for each potential face returned by the APIs.
The first is presented in the form of our null hypothesis:

$H_0:$ The potential face captured by the API is not an actual face.

$H_A:$ The potential face captured by the API is an actual face.

If the null hypothesis is true, the potential face location would not match the location of a manually annotated face. We would expect to see this for several reasons, when the face returned was deemed impractical or it would not be required that the chosen API would return the face in future use. Also when the potential face returned was a false discovery, in the instance where there was no face within the area deemed a potential face.

This compares to the alternative hypothesis, where the potential face location matched the location of a manual face. They were then given an identical ID to denote they captured the same face if the overlapping area of the two boxes was 1/3, or greater, of the potential face location area.


These definitions allowed us to define the four different scenarios of a hypothesis test. 

To consider how many Type II errors occurred, where an actual face was detected by an API, and incorrectly it was not annotated manually, and how many Type I errors occurred where a potential face was found by an API, and it was actually a face, but was not detected manually.
we look at the potential faces that do not match manually annotated faces.


```{r TypeMatchesManual, results='asis', fig.cap = "\\ label{tab:TypeMatchesManual}"}
...
We then considered that there would be an uneven amount of faces with certain image attributes. This is considered in the mosaics below in Figure \@ref(fig:Mosaic).
...
Tennis players faces will be the main focus of this use of facial detection. 
It is important that the selected API be able to return strong and consitent results for player's faces.

```{r Player, fig.cap = "This stacked bar chart allows consideration of the interaction between two image attributes. \\label{fig:DDPbgshot}"}
...
The use of accessories, Glasses and Visors or Hats, was considered as the Australian Open takes place on both indoor and outdoor courts. 
...
```{r Misc, fig.cap = "This stacked bar chart allows consideration of the interaction between two image attributes. \\label{fig:DDPbgshot}"}
