...
library(tidyverse)
library(purrr)
knitr::opts_chunk$set(echo=FALSE, message=FALSE, warning=FALSE, kfigr.link=TRUE, kfigr.prefix=TRUE, cache=FALSE, fig.env = TRUE, fig.cap=TRUE, fig.height=3)
...
This report aims to analyse the application of facial detection APIs to broadcasted video of Australian Open tennis matches to find a suitable API for face detection of players throughout a match.
Four APIs are contrasted based on the amount of faces returned, and whether these faces matched those found manually. Considerations of whether using a broadcast video of a sport is approporiate is based on the characteristics of the images in the sample. The characteristics of the scenes captured and the features associated with individual faces casts light on what would be most appropriate for accurate and high volume facial detection to occur. Situations where certain APIs are less capable are highlighted, should applications require reliable results in these situations the shortcomings in time taken and performance under less than optimal conditions would present certain APIs as inapplicable. 
...
While it is impossible to know the thoughts and feelings of a player during a match, professionals may be able to infer this information through results produced by a recognition software. This approach opposes the previous studies that have used player's recollections after a game to determine their emotions.
...
The aims of the present study were to determine the feasibility of using currently available APIs for extracting facial information of players during broadcasts of professional matches by comparing the performance of several popular facial recognition APIs. This limited selection was based on accessible APIs believed to produce appropriate and useful facial recognition. The performance of the evaluated software was compared against manual classifications obtained by an annotation tool developed by the authors. In addition to looking at the overall performance, image factors that influences the performance of each service were also investigated.
...
The process of software selection to determine which we would compare was based on several criteria. The results of the report, as it considered processing speed and feature selection techniques, as well as the ability to perform both still-to-video and video-to-video recognition.

From the software analysed we considered availability for use within the time frame of the report. This led us to choose Animetrics FaceR. The report outlines that for Animetrics, 'one requirement is that image/face proportion should be at least 1:8 and that at least 64 pixels between eyes are required'. We realize this could present challenges given the pixel sizes of faces in the image set. 
...
```{r Trial-Image, fig.cap = "This image of Bernard Tomic was chosen as a trial image to be presented to each of the software before they were included in the research. It was expected that the software would be able to find this face, despite the player facing away from the camera.\\label{fig:Trial-Image}", message=TRUE, warning=TRUE, dev="png", dpi=300}
...
A 'face' is considered to be any area designated manually or by an API as a location that encloses a human face. These may or may not be actual faces.

## Manual Annotations 

An annotation tool was constructed to allow image annotations that would capture the location of areas selected manually. We refer to the faces in these areas as Manual Face Classifications.
This tool also allowed selections of attributes of each face, and attributes describing the features of the Scenes. 
...
```{r UnlikelyFace, fig.cap = "The face captured manually in this image was not captured by any API. The head angle, shot angle, lighting, wearing the visor and being captured across the court during the match all contribute to the struggle of the APIs.\\label{fig:UnlikelyFace}", dev="png", dpi=300}
...
\begin{tabular}{ccc}
\hline  
Manual & Face & No Face\\ [0.5ex] 
%heading
\hline 
Face & 434 & 1861 \\
NoFace &527 & 787 \\ \hline
\end{tabular}
\caption{Animetrics}
}
\hfill
...
```{r UnusualImages, fig.cap = "Animetrics provided many results that were quite unusual. The top left image presents a potential faces that actually defines an area containing a player's back and a ball boy's torso. The top right shows two potential faces that are actually the KIA logo on the net. The bottom left image was very interesting as the player's face was found, but the shirts of both the player and ball boy were deemed potential faces. The bottom right image had a lot more faces that usually recognised. Animetrics found a fist to the right on the centre that it considered a potential face. \\label{fig:UnusualImages}", dev="png", dpi=300}
...
```{r MosaicShotangle, fig.cap = "\\label{fig:Mosaic}"}
...
The mosaic in \@ref(fig:ICFigs), Plot b shows the number of Faces captured during each possible situation. It contrasts how many of the images captured in situations either did or did not match the faces annotated manually using the colour pink for potential faces that did not match, and blue for those that did. It can be seen that the largest portion of the faces were captured in a close up situation while the players were on court.


```{r sbbgshot, fig.cap = "\\label{fig:sbbgshot}"}

sbbgshot <- ggplot(data=aminFaces) +
   geom_bar(aes(x=shotangle, fill = bg), 
            position = position_stack(reverse = TRUE), 
            na.rm=TRUE) + 
  scale_x_discrete(labels=c("Birds Eye"="Birds \nEye",
                            "Upward Angle"="Upward \nAngle",
                            "Player Shoulder Height" =
                            "Player \nShoulder \nHeight"))+
  coord_flip() +
  ...
  labs(caption="") +
  ylab("Amount of faces") + xlab("Angle the face was captured from") + theme(legend.position = "none") +
  ggtitle("Plot c. Proportion of faces captured\nat with each background given the angle")
...
```{r sbbgsit, fig.cap = "\\label{fig:sbbgsit}"}
...
plots <- arrangeGrob(MosaicShotangle, MosaicSit, sbbgshot, sbbgsit,
             nrow=2, ncol=2)


g <- ggplotGrob(sbbgshot + theme(legend.position="bottom"))$grobs
...
```{r accessories, fig.cap = "There are many more faces manually tagged as wearing glasses, and shows there was a more even split between those wearing headwear and those that were not.\\label{tab:accessories}"}
...
```{r glasses, fig.cap = "\\label{tab:glasses}"}

aminFaces %>%
  #filter(type=="Manual" | type=="Google") %>%
  droplevels(.) %>%
  CrossTable(data=., table(.$glasses, .$type), 
               prop.chisq = FALSE, 
               digits=2,
               expected = FALSE,
           prop.r = FALSE, prop.c = TRUE, prop.t = FALSE,
           drop.levels = TRUE, format = ("SPSS"), cell.layout = FALSE,
           dnn = c("Glasses","Headwear")) -> mgct

pander(mgct, caption = "This cross table compares the amounts and percentages of faces captured and whether the person was wearing glasses. The first row is the faces that were tagged as not wearing glasses, the second row is faces that were. For the API to not have been affected by the prescence of glasses, the proportion should be similar to that found manually. Animetrics had the highest proportio difference, 93.31% of the faces it found did not have glasses. Google is the closest proportion, 88.62% of the faces it found were wearing glasses.")
```
...
The APIs, the scripts used to access them and the resulting data set are contained in the supplementary materials.
...
Each script that was automated for POST requests created a classified faces csv.




## Packages
The R packages were used to create the following:
