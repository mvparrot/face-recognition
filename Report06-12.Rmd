...
overlayGgplot <- function(imgList, mergeData, matchBox=FALSE){
  for(i in imgList){
    ...
    if(matchBox){
      ...
      p <- p + geom_path(aes(x=x, y=450-y, group = c(FaceID), colour=type), faceData)
    }
  return(p)
  }
}
...
We were able to try the online demos to see whether these software were viable, we used the following Image displayed below in `r figr("Trial-Image", TRUE, type="Figure")`:



```{r Trial-Image, caption="This is a player", message=TRUE, warning=TRUE, anchor="figure"}
...
```{r Scene-Attributes-Table, anchor="table", caption="Attributes of a Scene of an image. The most appropriate option from the list was selected for each attribute."}
...
The program we created was an interactive application that worked by presenting an image, using the imager @imager package, from the sample of images that had not yet been considered, appearing underneath were a set of radio buttons corresponding to the Scene Attributes in `r figr("Scene-Attributes-Table", TRUE, type="Table")` as shown above.
...
####Analysis
The statistical analysis conducted to summarize and assess the validity of the method.
This method allowed all the software and the faces they found to be compared.


Using the data set^[ALLmetaIMG] of the combined API and manual results, we were able to compare the performance of the software.
Firstly, we considered how many individual faces the software were able to detect in `r figr("Face-Bounding-Box-Data", TRUE, type="Figure")` .


However, individual faces are not beneficial if they do not correspond to the faces manually annotated. `r figr("Face-Bounding-Box-UpSet", TRUE, type="Figure")` was created by defining groups depending on the softwares that recognized each particular face. The UpSetR [-@UpSetR] package helps visualise set intersections. Where in this circumstance Face Bounding Boxes may overlap on the same face, each bar shows the number of individual faces that have Face Bounding Boxes resulting from each of the softwares highlighted below the bar.


Following, we considered the sizes of the Boxes created in `r figr("Face-Bounding-Box-Data-Box", TRUE, type="Figure")`, this was an attempt to determine how the software performed when presented with smaller faces. The boxplot uses the size of the Face Bounding Boxes, and contrasts based on the software the Boxes were found from.
...
```{r Face-Bounding-Box-Data, results='hide', type="figure", caption="Face Bounding Box Data:", anchor="figure"}
...
The bar chart in 
`r figr("Face-Bounding-Box-Data", TRUE, type="Figure")` above shows the number of Bounding Boxes produced by each software, comparing the height of the bars indicates that Google's Facial Recognition software recognized almost 1000 more faces than the next best software, Microsoft.

-Google more than others

```{r Face-Bounding-Box-UpSet, results='hide', type="figure", caption="Face Bounding Box Data:", anchor="figure"}
createUpSet(ALLmetaIMGFaces)
```



`r figr("Face-Bounding-Box-Upset", TRUE, type="Figure")` Bar Chart shows the Face Bounding Boxes that were recognized by multiple software.
The largest group with 809 faces is Face Bounding Boxes only found by Manual annotations. The following groups were Boxes recognized by both Manual annotations and the Google API.



```{r Face-Bounding-Box-Data-Box, anchor="figure"}
ggplot(ALLmetaIMG, aes(x = factor(x = factor(type)), fill=type, y = size)) + geom_boxplot() + scale_y_continuous() +xlab("Facial Recognition Softwares")+scale_x_discrete(limits=c("Google","Animetrics","Manual","Microsoft","Skybiometry"))+ylab("Size of Face Bounding Boxes")+guides(fill=FALSE)
...
`r figr("Face-Bounding-Box-Data-Box", TRUE, type="Figure")` shows the differences between the sizes of the 'Face Bounding Boxes' produced by each software. Where a 'Face Bounding Box' refers to the size of the square the software provided as a location in the image of a face that was detected. On average, Google maps the largest boxes around faces, and has the largest box recognized in the set. On average, the smallest Face Boxes are created by Skybiometry.




```{r I2, results=TRUE, caption="2016_CT6_R01_JMillman_AUS_vs_DSchwartzman_ARG_MS159_clip.0014.png", anchor="figure"}
imagesList2<-as.list("2016_CT6_R02_FLopez_ESP_vs_GPella_ARG_MS227_clip.0009.png")
overlayGgplot(imagesList2, ALLmetaIMG, matchBox = FALSE)
```



The image above shows the smallest Face Bounding Box recognized. However visual inspection shows it only captures the player's eye and nose, not their whole face. 



```{r I3, results=TRUE, caption="2016_SC2_R01_ATomljanovic_AUS_vs_KBondarenko_UKR_WS1112_clip.0053.png", anchor="figure"}
...
```{r I4,results=TRUE, caption="2016_RLA_R04_NDjokovic_SRB_vs_GSimon_FRA_MS401_clip.0003.png", anchor="figure"}
...
```{r I5, results=TRUE, caption="2016_SC2_R01_AKerber_GER_vs_MDoi_JPN_WS1332_clip.0083.png", anchor="figure"}
...
`r figr("Image-Characteristics-Table", TRUE, type="Figure")`
 displays the feature combinations that produced the most potential face Bounding Boxes recognition by all four software. The most common shot is a crowd shot.
...
The best scenes for facial recognition have been found, given this information the following `r figr("Face-Characteristics-Table", TRUE, type="table")`
 considers the Characteristics of the individual faces found within those scenes. For this section we chose to consider only the faces that were manually annotated as players, with the intention of not basing results on recognition of undesirable faces.
