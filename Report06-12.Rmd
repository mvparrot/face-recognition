...
```{r GoogleRegression, fig.width=7.5, fig.cap = "Google Regression\\label{fig:GoogleRegression}"}
ALLmetaIMGnamedFaces %>% filter(type=="Manual" | type=="Google") %>%
  mutate(isplayer = ifelse(detect=="Player", 1, 0)) -> players

glm(isplayer ~ shotangle + bg + bg*shotangle + graphic + situation + lighting + glasses + visorhat, data=players) %>% summary()
